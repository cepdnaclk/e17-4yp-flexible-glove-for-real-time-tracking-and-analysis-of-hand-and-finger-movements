...
String compressRLE(String input) {
  ...
  for (unsigned int i = 1; i < input.length(); i++) {
    if (input[i] == lastChar) {
      ...
      result += lastChar; // Append the character
      result += String(count); // Append the count
      lastChar = input[i];
      count = 1;
    }
  }
...
// ----------------------------------------------------- socket init
const char* ssid = "Ishinis Dialog 4G";
const char* password = "Ishini123";
// const char* ssid = "mahelaekanayake10";
...
void dataCommunication(void* param) {
  ...
  // String compressed = compressRLE(total);
  // int com = compressed.length();
  // int to = total.length();
  // Serial.print("total len: ");
  // Serial.println(to);
  // Serial.print("compres: ");
  // Serial.println(com);
